...
There are two desirable outcomes for multiple edges on both sides, either elementwise edges or cross-product. The single line arrow (`→`) means element wise and double line arrow (`⇒`) means cross-product (don't tell anyone but for a single node on one side `→` is converted to `⇒` for convinience).
