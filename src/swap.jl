swap_node(x::T, old::T, new::T) where {T <: AbstractNode} = x == old ? new : throw(KeyError(old))
...
swap_edge(x::T, old::T, new::T) where {T <: AbstractEdge} = x == old ? new : throw(KeyError(old))
